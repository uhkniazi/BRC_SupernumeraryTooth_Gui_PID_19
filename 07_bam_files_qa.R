...
q = paste0('select Sample.id as sid, Sample.group1, Sample.group2, Sample.group3, Sample.title, Sample.description, File.* from Sample, File
           where (Sample.idData = 39) AND (File.idSample = Sample.id AND File.type like "%_sort%")')
...
setwd(gcswd)
n = make.names(paste('CBamScaffold pre duplicate removal data id 39 gui rds'))
lAllBams$meta = dfSample
lAllBams$desc = paste('CBamScaffold object from bam files before duplicate removal for gui', date())
...
#                 comment='CBamScaffold object from bam files before duplicate removal in rna seq data for gui and mouse data')
...
setwd(gcswd)
n = make.names(paste('CBamScaffold after duplicate removal data id 39 gui rds'))
lAllBams$meta = dfSample
lAllBams$desc = paste('CBamScaffold object from bam files after duplicate removal for gui', date())
n2 = paste0('~/Data/MetaData/', n)
save(lAllBams, file=n2)
...
# dbListFields(db, 'MetaFile')
# df = data.frame(idData=g_did, name=n, type='rds', location='~/Data/MetaData/',
#                 comment='CBamScaffold object from bam files after duplicate removal in rna seq data for gui and mouse data')
# dbWriteTable(db, name = 'MetaFile', value=df, append=T, row.names=F)
# dbDisconnect(db)
