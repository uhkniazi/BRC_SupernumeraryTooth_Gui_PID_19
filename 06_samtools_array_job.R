...
q = paste0('select File.*, Sample.idData from File, Sample 
           where (Sample.idData = 39) and (File.idSample = Sample.id) and (File.type = "fastq")')
...
cvQueries = paste0('select File.*, Sample.title from File, Sample 
                   where (Sample.idData = 39 and Sample.id =', dfQuery$SampleID, ') and (File.idSample = Sample.id) and 
                   (File.type = "fastq")')
