...
g_did
dfSample = dbGetQuery(db, "select * from Sample where idData=39;")
...
setwd(gcswd)
n = make.names(paste('CFastqQuality trimmed data id 39 gui rds'))
lOb$meta.1 = dfSample
lOb$meta.2 = dfFiles
lOb$desc = paste('CFastqQuality trimmed data id 39 gui', date())
...
df = data.frame(idData=g_did, name=n, type='rds', location='~/Data/MetaData/', comment='trimmed FASTQ quality checks on Gui mouse data')
...
pdf(file='results/qa.trimmed_fastq.pdf')
...
matplot(mQuality, type='l', main='Trimed base quality', ylab = 'Mean Score', xlab='Position in Read')
